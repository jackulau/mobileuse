...
_TMP = Path(ANH_TMP_DIR or default_runtime_base())
_RUNTIME = Path(ANH_RUNTIME_DIR or default_runtime_base())
...
def _default_endpoint(name):
    """Transport used when no ANH_BIND/ANH_CONNECT override is set.
...
def bind_endpoint(name):
    """Server-side endpoint. ANH_BIND overrides; default = unix on POSIX / tcp loopback on Windows."""
...
def connect_endpoint(name):
    """Client-side endpoint. ANH_CONNECT overrides; default = unix on POSIX / tcp loopback on Windows."""
...
def expected_token(name=None):
    ...
    ANH_TOKEN env wins. Otherwise: TCP transports get a per-name secret
    ...
    env = os.environ.get("ANH_TOKEN")
    if env:
        return env
    name = name or os.environ.get("ANH_NAME", "default")
...
def client_token(name):
    ...
    ANH_TOKEN env wins (remote clients set it by hand). For TCP transports the
...
def authorize(req, name):
    ...
    return {
        "error": ("auth required: this daemon speaks TCP and every request "
                  "must carry its token. Set ANH_TOKEN to the daemon's token "
    ...
    if ep[0] == "unix":
        ...
        if host not in _LOOPBACK:
            print(
                ...
                f"(remote clients: set ANH_TOKEN to the contents of "
